    ...
    <>
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        <Pressable className="flex-1 bg-black/40 items-center justify-center px-10" onPress={() => setOpen(false)}>
          {/* className no funciona sobre Animated.View (ver nota en Dialog.tsx) */}
          <Animated.View entering={FadeIn.duration(180).springify()}>
            <Pressable className="bg-white rounded-2xl p-5 gap-3 w-full max-w-xs" onPress={(e) => e.stopPropagation()}>
